        ...
        setCIeditForm:function(){
            ...
            $.each(data['findetails']['owner_details'], function( index, value ) {
                ...
                if(index=== 0 ||  index==='0')
                {
                    ...
                    that.set("Owner_Civic",value['own_civic']);
                    that.set("Owner_StreetAddress",value['own_street']);

                    that.set("state_user",value['own_state']);

                    createCityCmbEdit('',value['own_state'] , value['own_city']);

                    that.set("cmbCity",value['own_city']);

                    that.set("OwnZipCode",value['own_zip']);
                    var oDob = value['own_dob'].split('-');
                    that.set("owner_month",Number(oDob[1]));
                    that.set("owner_day",oDob[2]);

                    that.set("owner_year",oDob[0]);
                    that.set("own_percent",value['own_percent']);
                ...
                else
                {
                    ...
                    viewCModel.set("OwnerCivic"+index,(value['own_civic']!== 'undefined') ? value['own_civic'] : "");
                    viewCModel.set("OwnerStreetAddress"+index,(value['own_street']!== 'undefined') ? value['own_street'] : "");
                    viewCModel.set("own_state"+index,(value['own_state']!== 'undefined') ? value['own_state'] : "");
                    createCityCmbEdit(index,value['own_state'] , value['own_city']);
                    ...
                    viewCModel.set("owner_month"+index,(Number(own_DOB[1])!== 'undefined') ? Number(own_DOB[1]) : "");
                    viewCModel.set("owner_day"+index,(own_DOB[2]!== 'undefined') ? own_DOB[2] : "");
                    viewCModel.set("owner_year"+index,(own_DOB[0]!== 'undefined') ? own_DOB[0] : "");
                    
                    viewCModel.set("own_percent"+index,(value['own_percent']!== 'undefined') ? value['own_percent'] : "");
